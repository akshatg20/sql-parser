	...
	if comp2 == '<=':
		...
		if year >= max_year:
			if onlyYearQuery:
				...
				for (_, trie, _) in year_trie_index:
					diskloc_list.extend(execute_name_index(name, comp1, trie, stats))
				return diskloc_list
		...
		if valid_year is not None: 
			...
			for i in range(end_idx+1):
				if onlyYearQuery:
					diskloc_list.extend(year_trie_index[i][1].disk_records_locations_all())
				else:
					diskloc_list.extend(execute_name_index(name, comp1, year_trie_index[i][1], stats))
		...
		if valid_year is not None:
			# Find the index of this valid year
			start_idx = YearIndex.search_year(valid_year, year_trie_index, True) 
		...
		if valid_year is not None and valid_year == year :
			...
			if onlyYearQuery:
				...
				diskloc_list = execute_name_index(name, comp1, year_trie_index[year_idx][1], stats)
...
def execute_collapsed_trie_index(name, year, comp1, comp2, collapsedtrie, onlyNameQuery = False):
	...
	if comp1 == 'LIKE':
		prefix_name = name[1:-2]
		if onlyNameQuery:
			diskloc_list = collapsedtrie.disk_records_locations_name(prefix_name)
		else:
			diskloc_list = collapsedtrie.disk_records_locations(prefix_name, year, comp2, True)
	else:
		prefix_name = name[1:-1]
		if onlyNameQuery:
			diskloc_list = collapsedtrie.disk_records_locations_exact_name(prefix_name)
		else:
...
################################
	...
	if len(clause) == 1:
		...
		if clause[0][0] == 'year':
			year = int(clause[0][2])
			diskloc_list = execute_year_index(year, clause[0][1], year_index, ge_list, le_list, stats, stats[2])
		...
		# In case of queries like "WHERE name [comp] [NAME_VALUE]"
			...
			name = clause[0][2]
			diskloc_list = execute_name_index(name, clause[0][1], trie_index, stats)
			# diskloc_list = execute_collapsed_trie_index(name, 0, clause[0][1], "", collapsedtrie, True)
	...
	# for joint query
		...
		diskloc_list = execute_year_trie_index(name, year, clause[0][1], clause[1][1], year_trie_index, ge_list, le_list, stats)
