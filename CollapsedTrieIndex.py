...
class CollapsedTrie:
	...
	def index_graduation_year_collapsed(self, years_with_ids, gap=0):
		...
		for i, (_, year) in enumerate(sorted_years_with_ids):
			if year not in year_encountered:
				...
				year_start_index.append((year, i+gap))  # Record the first occurrence of the graduation year as (year, first_index, last_index)
		...
		while match_count < word_size:
			for child in current_node.children:
				...
				if word[match_count:]==child.word:
					...
					# some part of the word is common while some end words dont match
					prefix = self.longest_common_prefix(word[match_count:], child.word)
					match_count+=len(prefix)
					current_node = child
					return current_node
	...
	def disk_records_map(self, gap=0):
		...
		while stack:
			...
			sorted_ids, year_start_index = self.index_graduation_year_collapsed(current_node.word_ids, gap)
		...
		if node.year_start_index:
			...
			if comparator == '<=':
				...
				# elif year < list(node.year_start_index.keys())[0]:
					...
					valid_year_idx = YearIndex.upper_bound(node.year_start_index, year+1)
					if valid_year_idx is None:
						block_ids.extend(list(range(node.rank, node.rank + node.word_count)))
					else:
						block_ids.extend(list(range(node.rank, node.rank + valid_year_idx)))
				...
				# elif year > list(node.year_start_index.keys())[-1]:
					...
					valid_year_idx = YearIndex.upper_bound(node.year_start_index, year)
					block_ids.extend(list(range(node.rank + valid_year_idx, node.rank + node.word_count)))

				# for child in node.children:
				# 	block_ids.extend(self.pre_order_traversal(child, year, comparator))

			else:
				valid_year_idx = YearIndex.search_year(year, node.year_start_index)
				if valid_year_idx is not None:
					next_valid_year_idx = YearIndex.upper_bound(node.year_start_index, year + 1)
					if next_valid_year_idx is None:
						block_ids.extend(list(range(node.rank + valid_year_idx, node.rank + node.word_count)))
					else:
						block_ids.extend(list(range(node.rank + valid_year_idx, node.rank + next_valid_year_idx)))
